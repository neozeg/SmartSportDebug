	...
	*/
	...
	public void writeValue(BluetoothGattCharacteristic characteristic,byte[] data) {
		if (characteristic == null) {
			...
			mBluetoothLeService.sendCharacteristicData(characteristic, data);
		}
	}
			...
			if(gattService.getUuid().toString().contains(serviceUUID)){
				for(BluetoothGattCharacteristic gattCharacteristic:gattService.getCharacteristics()){
					if(gattCharacteristic.getUuid().toString().contains(charaUUID)){
						mBluetoothLeService.sendCharacteristicData(gattCharacteristic, data);
					}
				}
			}
		}		
		...
		for(BluetoothGattService gattService:mBluetoothLeService.getSupportedGattServices()){
				for(BluetoothGattCharacteristic gattCharacteristic:gattService.getCharacteristics()){
					...
					if(prop == BluetoothGattCharacteristic.PROPERTY_NOTIFY){
						return mBluetoothLeService.setCharacteristicNotification(gattCharacteristic, enabled);
					}
				}
		}
	...
	public boolean setCharacteristicNotification(BluetoothGattCharacteristic characteristic, boolean enabled){
		if(characteristic != null){
			return mBluetoothLeService.setCharacteristicNotification(characteristic, enabled);
		}
		return false;
	}
